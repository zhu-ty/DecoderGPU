...
namespace npp {
	...
	*/
	int NPPJpegCoder::decode(unsigned char* jpegdata, size_t input_datalength,
		...
		{
			NPP_CHECK_NPP(nppiYUV420ToRGB_8u_P3AC4R(apSrcImage, aSrcImageStep, outimg.data, outimg.step,
				osize));
			//cudaMemcpy2D(outimg.data, outimg.step, apSrcImage, aSrcImageStep, osize.width, osize.height, cudaMemcpyDeviceToDevice);
		}
	...
	int NPPJpegCoder::decode(unsigned char * jpegdata, size_t input_datalength, std::vector<void *> &_gpu_YUV420data, int &Ystep)
	{
		NppiDCTState *pDCTState;
		NPP_CHECK_NPP(nppiDCTInitAlloc(&pDCTState));
		...
		_gpu_YUV420data[0] = apSrcImage[0];
		_gpu_YUV420data[1] = apSrcImage[1];
		_gpu_YUV420data[2] = apSrcImage[2];
