...
void NvEncoderCuda::CopyToDeviceFrame_YUV420(
	CUcontext device,
	...
	bool bUnAlignedDeviceCopy)
{
	if (pixelFormat != NV_ENC_BUFFER_FORMAT_IYUV)
	{
		printf("error!");
		exit(-1);
	}
	...
	{
		m.srcDevice = (CUdeviceptr)pSrcFrames[0];
	}	
	m.srcPitch = srcPitch;
	m.dstMemoryType = CU_MEMORYTYPE_DEVICE;
	m.dstDevice = pDstFrame;
	...
	for (uint32_t i = 0; i < numChromaPlanes; ++i)
	{
		if (chromaHeight)
		{
			...
			{
				m.srcDevice = (CUdeviceptr)(pSrcFrames[i + 1]);
			}
